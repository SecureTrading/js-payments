...
behave-browserstack-real-device:
	$(DOCKER_COMPOSE) run -e DEVICE="$(DEVICE)" -e REAL_MOBILE=true -e OS_VERSION="$(OS_VERSION)" -e BS_ACCESS_KEY=$(BROWSERSTACK_ACCESS_KEY) -e BS_USERNAME=$(BROWSERSTACK_USERNAME) -e BS_LOCAL_IDENTIFIER=$(GITHUB_RUN_ID) -e BUILD_NAME=$(GITHUB_RUN_ID) -e REMOTE=true tests poetry run behave features --tags=$(BEHAVE_TAGS)

BROWSERSTACK_CHECK_ENV=BROWSERSTACK_USERNAME=$(BROWSERSTACK_USERNAME) BROWSERSTACK_ACCESS_KEY=$(BROWSERSTACK_ACCESS_KEY)
