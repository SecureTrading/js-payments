...
describe('ConfigService', () => {
  ...
  describe('clear', () => {
    it('sets config-provider value to null and publishes it to message bus', () => {
      spyOn(messageBus, 'publish');
  ...
  describe('getConfig$()', () => {
    ...
    it('returns config-provider as observable once', done => {
      configService
        .getConfig$(false)
        .pipe(take(3), toArray())
        .subscribe(result => {
    ...
    it('returns config-provider changes as observable', done => {
      configService
        .getConfig$(true)
        .pipe(take(3), toArray())
        .subscribe(result => {
