...
each([[{}, {default: null, minMatch: 0, maxMatch: 6, supported: ["AMEX", "ASTROPAYCARD", "DINERS", "DISCOVER", "JCB", "LASER", "MAESTRO", "MASTERCARD", "PIBA", "VISA"]}],
      ...
      [{defaultCardType: "AMEX", supported: ["AMEX", "MASTERCARD"]}, {default: brandMapping["4"], supported: ["AMEX", "MASTERCARD"]}],
...
.test("BinLookup.constructor", // Check different options get set on config correctly
   ...
   if (expected instanceof Object) {
       ...
       expect(() => {new BinLookup(testConfig)}).toThrow(expected);
   }
     });
...
test("BinLookup.getAllBrands",
   ...
   expect(bl.getAllBrands()).toEqual(["AMEX", "ASTROPAYCARD", "DINERS", "DISCOVER", "JCB", "LASER", "MAESTRO", "MASTERCARD", "PIBA", "VISA"]);
     });
